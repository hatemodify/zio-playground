...
import BurgerStack, { IngredientPicture } from '@/components/games/BurgerStack';
import { RewardCelebration } from '@/components/features';
import { useSound } from '@/hooks/use-sound';
import { useGameLogic } from '@/hooks/use-game-logic';
import { INGREDIENTS, RECIPES, paletteFor, type Recipe, type IngredientId } from '@/data/recipes';
...
export default function FoodStackGamePage() {
  ...
  const palette = !creative && difficulty === 'easy' && recipe ? paletteFor(recipe) : Object.values(INGREDIENTS);

  function start() {
    const pool = RECIPES.filter((item) => difficulty === 'easy' ? item.layers.length <= 5 : difficulty === 'normal' ? item.layers.length >= 5 && item.layers.length <= 6 : item.layers.length >= 5);
    const chosen = shuffled(pool).slice(0, difficulty === 'easy' ? 3 : difficulty === 'normal' ? 4 : 5);
  ...
  return <AdventureFrame title="차곡차곡 햄버거 가게" subtitle="주문서를 보고, 아랫빵부터 하나씩 쌓아요">
    {game.state === 'ready' ? <AdventureIntro picture="burger" title="오늘은 내가 햄버거 요리사!"
      instructions={creative ? ['좋아하는 재료를 마음대로 골라 쌓아요.', '한 칸 되돌리기로 모양을 바꿀 수 있어요.', '재료를 3개 이상 올리면 나만의 버거 완성!'] : ['주문서의 숫자를 따라 재료 그림을 살펴봐요.', '1번 아랫빵부터 차곡차곡 쌓고 마지막에 윗빵을 덮어요.', '완성한 햄버거를 손님께 드려요. 시간 제한은 없어요.']}
      difficulty={difficulty} onDifficulty={setDifficulty} onStart={start} showDifficulty={!creative}>
      <div className="mt-4 flex gap-2" role="group" aria-label="요리 방법"><button className="adventure-secondary" aria-pressed={!creative} onClick={() => setMode('orders')}>주문대로 만들기</button><button className="adventure-secondary" aria-pressed={creative} onClick={() => setMode('creative')}>내 마음대로 만들기</button></div>
    </AdventureIntro> : finished ? <section className="adventure-intro text-center">
      <BurgerStack layers={layers} compact /><h2 className="text-2xl font-extrabold">{creative ? '나만의 버거가 완성됐어요!' : `${recipes.length}개의 주문을 모두 완성했어요!`}</h2>
      <p className="mt-3 text-slate-600">{creative ? '재료를 쌓아 멋진 모양을 만들었어요.' : '아랫빵 → 속 재료 → 윗빵! 차례를 따라 끝까지 잘 만들었어요.'}</p>
      ...
      <div className="flex items-center justify-between gap-2 px-4 py-3"><p className="text-sm font-extrabold tracking-wide text-amber-950">또리 버거 · 주문을 부탁해!</p><span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-teal-800">{creative ? '자유 요리' : `${round + 1} / ${recipes.length} 주문`}</span></div>
      <div className={`grid gap-3 px-3 sm:px-5 ${creative ? '' : 'grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]'}`}>
        ...
        <div className="flex min-w-0 flex-col justify-end rounded-2xl bg-[#f9ebd4] p-2"><p className="pt-2 text-center text-xs font-bold text-amber-900">{creative ? '내가 만드는 새로운 메뉴' : '나의 요리대'}</p><BurgerStack layers={layers} /><p className="pb-2 text-center text-xs font-bold text-amber-900">{complete ? creative ? '아직 더 쌓아도 좋아요!' : '맛있는 버거 완성!' : `${layers.length}개 재료를 쌓았어요`}</p></div>
      </div>
      ...
      <div className="burger-counter p-3 sm:p-5"><p className="mb-2 text-xs font-bold text-amber-950">신선한 재료 · 그림을 눌러 담아요</p><div className="grid grid-cols-3 gap-2 sm:grid-cols-5" role="group" aria-label="재료 고르기">{palette.map((ingredient) => <button key={ingredient.id} aria-label={`${ingredient.label} 놓기`} disabled={creative ? layers.length >= 10 : complete} className="burger-ingredient" onClick={() => place(ingredient.id)}><IngredientPicture id={ingredient.id} className="h-9 w-full" /><span className="text-xs font-bold">{ingredient.label}</span></button>)}</div>
        <div className="mt-3 flex gap-2"><button className="adventure-secondary" disabled={!layers.length} onClick={() => { setLayers((old) => old.slice(0, -1)); setFeedback('한 칸 되돌렸어요. 다시 골라요.'); }}>한 칸 되돌리기</button><button className="adventure-primary flex-1" disabled={!complete} onClick={serve}>{creative ? '내 버거 완성' : '손님께 드리기'}</button></div>
      </div>
    </section>}
  </AdventureFrame>;
}
