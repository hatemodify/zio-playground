...
export const RECIPES: Recipe[] = [
  ...
  ...(['mushroom', 'tomato', 'onion', 'bacon', 'egg'] as const).map((topping): Recipe => ({ kind: 'pizza', id: `pizza-${topping}`, name: `${INGREDIENTS[topping].label} 피자`, layers: ['dough', 'sauce', 'cheese', topping, 'lettuce'] })),
  ...(['egg', 'bacon', 'cheese', 'patty', 'onion'] as const).map((topping): Recipe => ({ kind: 'sandwich', id: `sandwich-${topping}`, name: `${INGREDIENTS[topping].label} 샌드위치`, layers: ['bread', 'lettuce', topping, 'tomato', 'bread'] })),
  ...([
  ...
  ] as IngredientId[][]).map((layers, index): Recipe => ({ kind: 'pancakes', id: `pancakes-${index}`, name: ['딸기 팬케이크', '바나나 팬케이크', '과일 팬케이크', '무지개 팬케이크', '크림 팬케이크'][index], layers })),
];
